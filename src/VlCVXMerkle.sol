...
contract VlCVXMerkle is Owned {
  ...
  // Map, per user and per token of total claimed
  ...
  function claim(address token, uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public {
    ...
    // Get the real claimable
    // ie : amount in params is the total to claim since inception
    uint256 claimable = _claimable(account, amount, token);
    claimed[account][token] = amount;

    ERC20(token).safeTransfer(account, claimable);

    emit Claimed(token, index, claimable, account, update[token]);
  }

  function claimable(address account, claimParam calldata claim) external view returns(uint256) {
    return _claimable(account, claim.amount, claim.token);
  }

  function _claimable(address account, uint256 amount, address token) internal view returns(uint256) {
    return amount - claimed[account][token];
  }
